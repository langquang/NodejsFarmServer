...
GameModel.getUserId = function (key) {
    return key.slice(5, key.length);
};

GameModel.create = function (uid, username, callback) {
    var gameDoc = new GameData(uid, username, 5000, 20, Helper.getSeconds(), 10);
...
GameModel.get = function (uid, callback) {
    ...
    db.get(key, function (err, result) {
        ...
        var gameDoc = new GameData(result.value.userId, result.value.username, result.value.gold, result.value.energy, result.value.lastEnergy, result.value.exp, result.value.lastLogin);
